App = {
      ...
      // console.log("account: "+ acct);
        ...
        console.log(cred)
          ...
          if (cred == "inspector"){
            ...
            sessionStorage.setItem("account-hex", acct);
    ...
    welcoming: function(){
      ...
      $.getJSON( "people.json", function(data) {
        ...
        for(var i=0; i<data.length; i++){
          ...
          if(sessionParsed == address ){
            $("#requireNameModal").hide();
            $("#welcome").show();
            name = data[i].owner;    
          }
        } 
        $("#welcome").html("Welcome, <span class='text-primary'>"+name+"</span>");
    ...
    encryptAndStore : async function(){
        //to be hashed (specific to land)
          ...
          console.log("Parse Land ID: "+ id);

          let combined2 = owner.toLowerCase() + ownerName.toLowerCase();
          combined2 = combined2.replace(/\s+/g, '');
          combined2 = combined2.split('.').join("");
          combined2 = combined2.split(',').join("");
          combined2 = combined2.split('-').join("");
          ...
          $.getJSON( "land.json", function(data) {
            ...
            if(data.length == 0){
              ...
              if(idsArray.indexOf(id) !== -1){
                $('.modal').hide();
                $('#redundant').fadeIn(500).delay(2000).fadeOut(2000);
                console.log("Exists");
              }
              else{
                App.storeProper(
      ...
      instance.Registration(
        ...
        $.getJSON( "people.json", function(data) { 
          ...
          if(data.length == 0){
            ...
            console.log("Owner ID Array: "+ idsArray);

            if(idsArray.indexOf(ownerId) !== -1){
              //console.clear();
              console.log("Owner already exists. Store no more.");
            }
  ...
  saveBuyer: function(){
    ...
    console.log("account: "+hexAccnt);
    hexAccnt = hexAccnt.toString();
    ...
    if(name === ""){
      alert("Do not leave empty fields.")
        ...
        let combined2 = name.toLowerCase() + hexAccnt.toLowerCase();
        combined2 = combined2.replace(/\s+/g, '');
        combined2 = combined2.split('.').join("");
        combined2 = combined2.split(',').join("");
        combined2 = combined2.split('-').join("");
        ...
        $("#requireNameModal").fadeOut(200);
        $(".alert-saved").fadeIn(200).delay(2000).fadeOut(2000);
      }
  },
    ...
    pullAssets: async function(element){
      ...
      console.log("holy acct: "+ account);
     
      let assets = await LanReg.viewAssets(account);
    // console.log(assets[i]);
      ...
      for(var i=0; i <assets.length; i++){
        //  console.log(assets[i].c[0]);
          ...
          let output = await LanReg.getTransaction(landId);
         ...
         // console.log(requester);
              ...
              if(!res[3] && requester != 0x0000000000000000000000000000000000000000){
                    ...
                    availableButton = "<button onclick='App.makeAvailable("+landId+")' id='availableButton' class='btn btn-info border-light' data-toggle='tooltip' data-placement='right' title='Make available for sale.'>Make Available</button>";
              }
              else{
          ...
          "<div class='card mb-2' style='width: 18rem;'>"+
            ...
            "<div class='card-body'>"+
              ...
              "<a href='title/certificate/manual/template2.html?"+landId+"' class='btn btn-info border-light'>View Title Certificate</a>"+
              "<button onclick='App.getValue("+landId+")' class='btn btn-info border-light'>Update Market Value</button>"+
               availableButton+
    ...
    viewRequest: async function(id){
        ...
        $.getJSON( "people.json", function(data) {
          ...
          let name;
          for(var i=0; i<data.length; i++){
            if(parseInt(data[i].address,16) == parsedReq ){
              name = data[i].owner;
            }
          }

        $('#requester').html("Requested by: "+ req[4] +"<p class='text-primary'>"+name+"</p>");
    ...
    hookBuyerAccount: function(){
      web3.eth.getAccounts(function(err, accounts){
        if (err != null){
          ...
          sessionStorage.setItem("account-hex", accounts[0]);
    ...
    buyAsset: async function(id, currentOwner){  
      ...
      try{
        App.contracts.LandRegistration.deployed().then(function(instance){
          ...
          let date = Date.now();
          date = App.handleDate(date);
          //date = App.convertUnixtoDate(date);

          console.log("PURCHASE DATE: "+ date);

          instance.buyProperty(id, date, {
            from: account,
            value: mValue
          });
        }).then(function(){
